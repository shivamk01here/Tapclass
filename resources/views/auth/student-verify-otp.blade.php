...
<head>
  ...
  <title>Verify your email - HTC</title>
...
<body class="bg-[#f6f7f8] font-display">
  <div class="min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md bg-white rounded-xl shadow p-6">
      ...
      <div class="space-y-4">
        ...
        <p class="text-sm text-gray-600">We sent a 6-digit code to <span class="font-medium">{{ $pending['email'] ?? '' }}</span>. Enter it below to complete your registration.</p>
        ...
        <form action="{{ route('register.student.verify-otp.verify') }}" method="POST" class="space-y-3">
          @csrf
      ...
      <div id="issue-modal" class="fixed inset-0 bg-black/40 hidden items-center justify-center">
        <div class="bg-white w-full max-w-sm rounded-lg p-4 fade-in">
          ...
          <form method="POST" action="{{ route('register.student.otp-issue') }}" class="mt-3 space-y-3">
            @csrf
      ...
      <script>
        setTimeout(function(){ window.location.href = "{{ route('student.dashboard') }}"; }, 1500);
      </script>
      @endif
    </div>
  </div>
  ...
  <script>
    ...
    // DEV: log OTP in console when available and app is in debug
