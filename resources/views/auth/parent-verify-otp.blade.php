...
<head>
  ...
  <title>Verify your email - HTC (Parent)</title>
  ...
  <div class="min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md bg-white rounded-xl shadow p-6">
      ...
      <div class="space-y-4">
        ...
        <form action="{{ route('register.parent.verify-otp.verify') }}" method="POST" class="space-y-3">
          @csrf
      ...
      <div id="issue-modal" class="fixed inset-0 bg-black/40 hidden items-center justify-center">
        <div class="bg-white w-full max-w-sm rounded-lg p-4 fade-in">
          ...
          <form method="POST" action="{{ route('register.parent.otp-issue') }}" class="mt-3 space-y-3">
            @csrf
      ...
      <div class="flex flex-col items-center text-center space-y-4">
        ...
        <p class="text-sm text-gray-600">You’ll be redirected shortly…</p>
      </div>
      <script>
        setTimeout(function(){ window.location.href = "{{ route('onboarding.parent.show') }}"; }, 1500);
      </script>
      @endif
    </div>
  </div>
